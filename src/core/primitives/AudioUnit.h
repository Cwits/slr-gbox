...
class AudioUnit {
    ...
    virtual frame_t process(const AudioContext &ctx, const Dependencies &inputs) const = 0;

    virtual void prepareToPlay() = 0;
    virtual void prepareToRecord() = 0;
    virtual void stopPlaying() = 0;
    virtual void stopRecording() = 0;

    virtual frame_t latency() { return 0; }
    ...
    ParameterBool _mute;
    mutable bool _buffersClear;
    bool isMuted(const AudioContext &ctx) const;
    ...
    void fetchAndSortMidi(const AudioContext &ctx, const Dependencies &inputs) const;
    mutable MidiBuffer * _midiInput;
    ...
    // void applyMidiEvents(MidiBuffer *buf) const;

    void playbackFiles(const AudioContext &ctx, AudioBuffer *buf, MidiBuffer *mid) const;
